...
The realised partition was similar across most recorded baseline variables: age (overall median 61 years, IQR 54 to 70; p=0.222), sex (130/188 male, 69.15%; p=0.709), smoking status (113/188 ever-smokers, 60.11%; p=0.688), duration of smoking, comorbid cancer, tuberculosis, bronchiectasis and fungal infection, antiplatelet and antithrombotic medication, platelet count (PLT), D-dimer (D-D), international normalised ratio (INR) and fibrinogen (Fib), none of which reached p<0.10 (Table 1).

One variable was the exception. Bronchial artery abnormality (BAA), a composite of bronchial artery–pulmonary fistula and bronchial artery malformation, which represent variants of the same abnormal bronchial arterial vasculature, was present in 38 of 188 patients overall (20.21%) but was distributed unevenly between the two sets: 21 of 131 training patients (16.03%) and 17 of 57 validation patients (29.82%), p=0.049. The validation cohort therefore carried nearly twice the prevalence of the strongest binary term in the model. Because the sets were generated by random splitting rather than by any allocation rule, this difference is a product of the partition obtained rather than of any systematic difference in ascertainment, but it should be kept in view when the two cohorts are compared.
...
Discrimination was good in the training cohort, with an area under the receiver operating characteristic curve (AUC) of 0.850 (95% CI 0.775 to 0.925), sensitivity 0.767 (23/30), specificity 0.842 (85/101) and accuracy 0.824 (108/131) at the 0.255 cut-point. In the internal validation cohort the AUC was 0.794 (95% CI 0.631 to 0.958), with sensitivity 0.750 (9/12), specificity 0.667 (30/45) and accuracy 0.684 (39/57) at the same cut-point (Table 3, Figure 6). Sensitivity was thus slightly higher and specificity appreciably lower in validation than in training, and overall accuracy fell from 0.824 to 0.684. The validation interval spans 0.327 AUC units and its lower bound lies close to the 0.5 no-discrimination line, so the validation result is compatible with performance ranging from marginal to excellent and should not be read as confirmation of the training estimate.
...
**Table 2.** Univariable and multivariable logistic regression. 保留**全部单因素行**：原 17 项中 BAPF 与 BAM 合并为单一 BAA 行，故为 16 行。**不保留 BAPF、BAM 的分开行**（BAA 为唯一设定）。单因素列全填，多因素列只填进入模型的 4 项，其余用破折号。⚠️ 补充材料中 Rad-score 单因素 p 显示为 `0.000`，应改写为 `<0.001`。`P` 与 `P-raw` 只需保留精确值一列。

**Table 1.** Baseline characteristics. **删除 BAPF、BAM 的分开行**，只保留 BAA 一行（全体 38/188, 20.21%；训练 21/131, 16.03%；验证 17/57, 29.82%；p=0.049）。表注：`BAA, bronchial artery abnormality, comprising bronchial artery–pulmonary fistula (n = 27) and bronchial artery malformation (n = 18), with 7 patients carrying both.` 成分计数放表注，不进正文论证。
...
### 仍缺的数字 → ✅ 已于第二、三批补充材料全部补齐

- Rad-score 四组中位数与 IQR → Table 4 已提供，与箱线图逐一核对相符。
- 验证集精确 p = **0.122**；训练集精确 p = **1.49 × 10⁻⁵**。
- 检验方法 = **Mann–Whitney U**。
...
### 合并的定位（2026-08-10 作者确认后更新）

作者确认：**未合并 BAPF/BAM 的那一版才是不规范的**，合并后为唯一正确设定。据此调整全文处理方式：

- **合并理由改为纯解剖学表述**：BAPF 与 BAM 是同一支气管动脉血管异常的两种表现形式。**正文不再引用「两者单因素 OR 相近（4.88 vs 3.84）」作为理由**——那属于经验性依据，暗示先分开分析过，与「合并是正确设定」的定位自相矛盾。
- **Table 1、Table 2 只保留 BAA 行**，删除 BAPF、BAM 分开行；成分计数（27 / 18 / 兼有 7）移入 Table 1 表注。
- **正文不写 `a priori` / `pre-specified`**。那是关于「何时决定」的事实主张，我无法核实；而纯定义句在任何情形下都成立，同样满足「只用合并后数据」的要求。论文只需交代最终设定，不必叙述分析过程中的调整。
- 原先「必须交代合并时序」的要求随之取消。

**命名**：采用 **bronchial artery abnormality (BAA)**（作者选定）。列线图轴标仍是「BAPF」，需改。
...
| 项目 | 核验 |
|---|---|
| 验证集操作点 | sens 0.750 × 12 = **9/12**；spec 0.667 × 45 = **30/45**；(9+30)/57 = **0.6842** ✅ 与所报 0.684 完全吻合 |
| BAA 合并计数 | 27 (BAPF) + 18 (BAM) − 38 (合并) = **7 例同时具备两种病变**；训练 15+10−21=4，验证 12+8−17=3，4+3=7 ✅ 自洽 |
...
补充材料写：「HL 检验在两个数据集均不显著，indicating no evidence of poor model fit and **suggesting acceptable calibration in both datasets**」。

被同一份材料的图直接证伪：验证集**截距 −0.781、斜率 0.741、Emax 0.237、Eavg 0.111**，logistic 校准曲线**全程**位于对角线下方。这不是 acceptable calibration，是系统性高估。HL 在 12 个事件下几乎不可能显著，阴性结果属效能不足，不能反推校准良好。

**这与第一轮修正的是同一类错误**（当时原稿称「曲线全范围贴合对角线」）。正文已按图中数值如实改写，未采用该句。

### ⚠️ P7（M）：合并 BAM 后验证集校准变差
...
**判别力上升、校准度下降**，方向相反。这不矛盾（AUC 只看排序，校准看绝对水平），但值得在 Discussion 说明一句。按你的选择，正文如实报告、不额外跑再校准。

### ⚠️ P8（M）：Table 1 的 BAA 行 p=0.049

训练 16.03% vs 验证 29.82%，是全表**唯一**达到显著的基线差异。原稿正文「所有基线变量均无差异」的表述已相应改写。

成因是随机划分的偶然结果（合并前 BAPF 单独 p=0.134、BAM 单独 p=0.271，都不显著，合并后叠加才达到 0.049），但**验证集该变量患病率接近训练集两倍**，这可能是验证集校准偏差与特异度下降的原因之一。已写入正文，建议 Discussion 呼应。

### ⚠️ 其余小项

- 补充材料中 Rad-score 单因素 p 显示为 `0.000` → 应写 `<0.001`。
- 新列线图第二行轴标仍是「BAPF」→ 按你的选择应改为 **BAA**。
- 列线图分值（Rad-score 0–100、fib 0–约 55、TB≈23、BAA≈21、总分 0–180）是我从图中读取的，请与 `nomogram()` 输出核对。
...
| **Fig 4** 森林图 | ✅ **代码已提供** `fig4_forest.R` | 数值已硬编码（16 行单因素 + 4 行多因素，BAA 单行），**无需数据即可直接运行**。对数横轴，两面板统一风格，右侧带 OR (95% CI) 与 P 两列。内置断言：行数、BAA 存在、BAPF/BAM 不存在、CI 包含点估计 |
...
4. ~~**ICC 的实际取值与形式**~~ → ✅ **已全部解决**（2026-08-10）：保留特征 ICC **0.760–0.914，中位数 0.849**；形式为**双向随机效应、绝对一致性、单次测量**，即 ICC(2,1)（Shrout & Fleiss 记法）/ ICC(A,1)（McGraw & Wong 记法）。按 Koo & Li (2016) 分级，绝大多数落在「良好」区间（0.75–0.90），最高值刚触及「优秀」下限——如实报告即可。

   **Methods 可直接粘用的句子：**

   > Inter-reader reproducibility of the extracted features was assessed in a random subset of 20 patients segmented independently by both radiologists. Agreement was quantified with the intraclass correlation coefficient (ICC) under a two-way random-effects model for absolute agreement using single measurements (ICC(2,1)). Features with an ICC of 0.75 or below were discarded.

   选双向随机而非二元混合是恰当的：前者把两位阅片者视作可能阅片者总体的随机样本，结论可外推到其他阅片者；后者的结论仅对这两位成立，对可重复性论证的支持更弱。
...
6. ~~**验证集校准的处理方案**~~ → ✅ **作者已选 (a) 如实报告 + 写入 Limitations**。新拟合下截距 −0.781、斜率 0.741，比合并前更差，正文已如实呈现。
7. ~~**单变量 ROC 图的 AUC 数值**~~ → ✅ **已按图注表述解决**。新四联图的 (a)(b) 面板同样不显示各变量 AUC，修订后的图注不再作此承诺，图文一致。
7b. ~~**AUC 版本核对**~~ → ✅ **已定版**：0.850 / 0.796 / 0.794，根因为 BAM 并入 BAPF 的重新拟合。全部连带数据已回填，**正文数值到此全部锁定**。
