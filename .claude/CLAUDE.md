...
## Path resolution in this repository

ARS is vendored here as **project skills** (upstream SETUP.md Method 1), not as a plugin.
Everything below `.claude/skills/` keeps the upstream repository layout, so
`.claude/skills/` is the ARS root: paths written as `shared/...`,
`scripts/...`, `academic-paper/SKILL.md`, or `MODE_REGISTRY.md` in any skill,
agent, or reference file resolve relative to `.claude/skills/`.

Two things differ from an upstream plugin install:

- **Hooks are not installed.** Upstream `hooks/hooks.json` wires a SessionStart
  announce and the `ars_write_scope_guard.py` PreToolUse guard through
  `${CLAUDE_PLUGIN_ROOT}`, which only resolves for plugin installs. The guard is
  optional v3.10 hardening and ARS core does not need it.
- **Test files are omitted.** `scripts/test_*.py`, `scripts/fixtures/`, and
  `scripts/adapters/tests/` were dropped; the runtime checkers the skills call
  are all present.
