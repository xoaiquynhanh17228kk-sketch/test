...
- **英文补充主题词**（对齐现场重点，服务后续按环节编码）：`school tuberculosis China`、`student TB screening`、`TPT adolescent`、`contact investigation`、`cost-effectiveness school TB`。
- **政策/灰色文献库**：WHO IRIS、国家卫健委、教育部、中国疾控中心政策库站内检索"学校结核病 规范/指南"；重庆市卫健委/疾控实施细则；学位论文库单独跑一轮。

> **编码原则**：纳入后**按环节编码**（入学/年级筛查、症状监测与诊断延误、密接筛查、潜伏感染与 TPT、休复学、治疗管理与基本公卫衔接、卫生经济学、部门治理与信息流转），**不按"阳性率"堆砌**。产出应包含"已有结论 / 必须现场补"对照表，供 `07` 指标词典与 `05` 现状调查取用。
