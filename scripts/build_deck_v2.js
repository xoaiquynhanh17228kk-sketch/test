...
// —— 2 汇报提纲 ——
{
  const s = contentSlide({
    title: "汇报提纲", lead: "按《重庆医科大学研究生学位论文开题报告评分指标》的五个考核板块组织",
    notes: "开门见山说明：本次汇报的顺序与各位手上评分表的板块顺序一致，方便对照打分。分值最高的是第四部分文献综述及研究基础（30 分），其中包含已完成的综述与预实验结果。",
  });
  cards(s, [
    { tag: "A　20 分", head: "一、选题依据", body: "选题来源 · 研究内容与重点解决的问题\n理论意义及应用价值 · 预期目标", tagColor: TEAL, headColor: TEAL },
    { tag: "B　20 分", head: "二、研究方法", body: "拟采取的研究方法 · 技术路线\n总体安排与计划进度", tagColor: NAVY, headColor: NAVY },
    { tag: "C　20 分", head: "三、研究条件", body: "学术或技术条件 · 实验设备与平台\n经费概算及落实情况", tagColor: GOLD, headColor: GOLD },
    { tag: "D　30 分　分值最高", head: "四、文献综述及研究基础", body: "本领域发展动态 · 文献查阅情况\n已做调研情况（含初步研究结果）", tagColor: RED, headColor: RED },
    { tag: "E　10 分", head: "五、总结", body: "表达能力 · 总结与提炼能力", tagColor: MUTED, headColor: MUTED },
    { tag: "", head: "本次汇报约 25 分钟", body: "各部分时长按分值配比：\nA 5 min · B 9 min · C 2 min · D 8 min · E 1 min", tagColor: MUTED, headColor: INK, headSize: 13 },
...
// —— 分节页 A ——
{
  const s = darkSlide();
  pageNo += 1;
  s.addShape(pres.ShapeType.roundRect, { x: M + 0.3, y: 2.35, w: 1.05, h: 0.95, rectRadius: 0.10, fill: { color: "1D7874" } });
  s.addText("A", { x: M + 0.3, y: 2.35, w: 1.05, h: 0.95, margin: 0, fontFace: FONT, fontSize: 34, bold: true, color: WHITE, align: "center", valign: "middle" });
  s.addText("一、选题依据", { x: M + 1.7, y: 2.42, w: 8.4, h: 0.62, margin: 0, fontFace: FONT, fontSize: 30, bold: true, color: WHITE, valign: "middle" });
  s.addText("评分 20 分", { x: M + 1.75, y: 3.06, w: 9.5, h: 0.42, margin: 0, fontFace: FONT, fontSize: 14, color: "9FC3D8", valign: "middle" });
  s.addText("临床问题 · 切入点 · 科学假说 · 关键问题 · 研究内容 · 创新点 · 预期目标", { x: M + 1.75, y: 3.62, w: 9.5, h: 0.42, margin: 0, fontFace: FONT, fontSize: 12.5, color: "7FB3CC", valign: "middle" });
  s.addText(String(pageNo), { x: W - M - 0.9, y: H - 0.62, w: 0.9, h: 0.32, margin: 0, fontFace: FONT, fontSize: 10, color: "7FB3CC", align: "right" });
  s.addNotes("第一部分，约 5 分钟。对应评分表 A 栏的四项细目：选题来源、研究内容及重点解决的问题、理论意义及应用价值、预期目标。");
}
...
// —— 4 平面化困境 ——
{
  const s = contentSlide({
    ...
    notes: "这一页是为第 6 页的切入点做铺垫。核心一句：现有研究回答了『发生了什么』，没回答『从何处启动、为何在特定细胞器区域被放大』。",
  });
  bullets(s, [
...
// —— 分节页 B ——
{
  const s = darkSlide();
  pageNo += 1;
  s.addShape(pres.ShapeType.roundRect, { x: M + 0.3, y: 2.35, w: 1.05, h: 0.95, rectRadius: 0.10, fill: { color: "0B3C5D" } });
  s.addText("B", { x: M + 0.3, y: 2.35, w: 1.05, h: 0.95, margin: 0, fontFace: FONT, fontSize: 34, bold: true, color: WHITE, align: "center", valign: "middle" });
  s.addText("二、研究方法", { x: M + 1.7, y: 2.42, w: 8.4, h: 0.62, margin: 0, fontFace: FONT, fontSize: 30, bold: true, color: WHITE, valign: "middle" });
  s.addText("评分 20 分", { x: M + 1.75, y: 3.06, w: 9.5, h: 0.42, margin: 0, fontFace: FONT, fontSize: 14, color: "9FC3D8", valign: "middle" });
  s.addText("技术路线 · 三队列 · 形态学 · 组学 · 因果验证 · 统计 · 风险 · 进度", { x: M + 1.75, y: 3.62, w: 9.5, h: 0.42, margin: 0, fontFace: FONT, fontSize: 12.5, color: "7FB3CC", valign: "middle" });
  s.addText(String(pageNo), { x: W - M - 0.9, y: H - 0.62, w: 0.9, h: 0.32, margin: 0, fontFace: FONT, fontSize: 10, color: "7FB3CC", align: "right" });
  s.addNotes("第二部分，约 9 分钟，篇幅最大。对应评分表 B 栏：拟采取的研究方法、技术路线、总体安排与计划进度。");
}
...
// —— 20 TEM SOP ——
{
  const s = contentSlide({
    ...
    notes: "『全组配平铁律』是本页要留在评委脑子里的一句话。若被问 2D vs 3D EM，转到第 27 页风险表的 S1 主动交代。",
  });
  bullets(s, [
...
// —— 分节页 C ——
{
  const s = darkSlide();
  pageNo += 1;
  s.addShape(pres.ShapeType.roundRect, { x: M + 0.3, y: 2.35, w: 1.05, h: 0.95, rectRadius: 0.10, fill: { color: "B08333" } });
  s.addText("C", { x: M + 0.3, y: 2.35, w: 1.05, h: 0.95, margin: 0, fontFace: FONT, fontSize: 34, bold: true, color: WHITE, align: "center", valign: "middle" });
  s.addText("三、研究条件", { x: M + 1.7, y: 2.42, w: 8.4, h: 0.62, margin: 0, fontFace: FONT, fontSize: 30, bold: true, color: WHITE, valign: "middle" });
  s.addText("评分 20 分", { x: M + 1.75, y: 3.06, w: 9.5, h: 0.42, margin: 0, fontFace: FONT, fontSize: 14, color: "9FC3D8", valign: "middle" });
  s.addText("学术与技术条件 · 实验设备与平台 · 经费概算及落实", { x: M + 1.75, y: 3.62, w: 9.5, h: 0.42, margin: 0, fontFace: FONT, fontSize: 12.5, color: "7FB3CC", valign: "middle" });
  s.addText(String(pageNo), { x: W - M - 0.9, y: H - 0.62, w: 0.9, h: 0.32, margin: 0, fontFace: FONT, fontSize: 10, color: "7FB3CC", align: "right" });
  s.addNotes("第三部分，约 2 分钟。对应评分表 C 栏三项细目，讲快一点，重点在落实状态。");
}
...
// —— 29 平台与技术条件 ——
{
  ...
  table(s, ["所需平台 / 技术", "落实方式", "成熟度", "波动风险控制"], [
    ...
    ["行为学（MWM / NOR / 旷场）", "校内共享（已跑通，见第 40–43 页）", { t: "成熟", b: true }, "全程操作与分析双盲；范式修订已完成"],
...
// —— 分节页 D ——
{
  const s = darkSlide();
  pageNo += 1;
  s.addShape(pres.ShapeType.roundRect, { x: M + 0.3, y: 2.35, w: 1.05, h: 0.95, rectRadius: 0.10, fill: { color: "C0392B" } });
  s.addText("D", { x: M + 0.3, y: 2.35, w: 1.05, h: 0.95, margin: 0, fontFace: FONT, fontSize: 34, bold: true, color: WHITE, align: "center", valign: "middle" });
  s.addText("四、文献综述及研究基础", { x: M + 1.7, y: 2.42, w: 8.4, h: 0.62, margin: 0, fontFace: FONT, fontSize: 30, bold: true, color: WHITE, valign: "middle" });
  s.addText("评分 30 分 —— 分值最高", { x: M + 1.75, y: 3.06, w: 9.5, h: 0.42, margin: 0, fontFace: FONT, fontSize: 14, color: "9FC3D8", valign: "middle" });
  s.addText("检索策略 · 证据分级 · 竞争模型 · 已完成综述 · 预实验结果", { x: M + 1.75, y: 3.62, w: 9.5, h: 0.42, margin: 0, fontFace: FONT, fontSize: 12.5, color: "7FB3CC", valign: "middle" });
  s.addText(String(pageNo), { x: W - M - 0.9, y: H - 0.62, w: 0.9, h: 0.32, margin: 0, fontFace: FONT, fontSize: 10, color: "7FB3CC", align: "right" });
  s.addNotes("第四部分，约 8 分钟，分值最高。前半是文献综述，后半是研究基础（综述 + 预实验）。预实验四页是本部分的实证支撑。");
}
...
// —— 9 时间轴 ——
{
  const s = contentSlide({
    ...
    notes: "三个里程碑要能一口气讲下来。最后一条的补注是本页重点，也是与第 7 页假说表述纪律呼应的地方。",
  });
  cards(s, [
...
// —— 11 BHD 进展与不足 ——
{
  const s = contentSlide({
    ...
    notes: "最后一条是综述自己提出的判据，务必讲——它直接引出第 25 页 G9 vs G6 的设计（本版页码不变）。",
  });
  bullets(s, [
...
// —— 27 前期工作基础 ——
{
  ...
  s.addText([
    ...
    { text: "→ 提出振荡接触模型（第 17 页）　→ 建立四级证据分级（第 34 页）　→ 给出三模型证伪标准（第 37 页）", options: { fontFace: FONT, fontSize: 12, bold: true, color: INK } },
  ], { x: M + 0.22, y: BODY_TOP + 0.14, w: W - M * 2 - 0.44, h: 1.54, margin: 0, valign: "top", lineSpacing: 19 });

  cards(s, [
    { tag: "① 文献调研", head: "", body: "三库四组检索词交叉检索（见第 33 页）\n形成三条证据链梳理与研究空白定位\n核心引用【填】篇", headSize: 1, tagColor: TEAL },
  ...
  s.addText([
    ...
    { text: "已完成 Pilot V4.5 赛道 A 体内预实验（Sham / CIH / CIH+BHD 各 n=3，CIH 4 周）—— 造模成立　→ 详见后四页", options: { fontFace: FONT, fontSize: 12.5, color: INK } },
...
// —— 分节页 E ——
{
  const s = darkSlide();
  pageNo += 1;
  s.addShape(pres.ShapeType.roundRect, { x: M + 0.3, y: 2.35, w: 1.05, h: 0.95, rectRadius: 0.10, fill: { color: "5A6B7B" } });
  s.addText("E", { x: M + 0.3, y: 2.35, w: 1.05, h: 0.95, margin: 0, fontFace: FONT, fontSize: 34, bold: true, color: WHITE, align: "center", valign: "middle" });
  s.addText("五、总结", { x: M + 1.7, y: 2.42, w: 8.4, h: 0.62, margin: 0, fontFace: FONT, fontSize: 30, bold: true, color: WHITE, valign: "middle" });
  s.addText("评分 10 分", { x: M + 1.75, y: 3.06, w: 9.5, h: 0.42, margin: 0, fontFace: FONT, fontSize: 14, color: "9FC3D8", valign: "middle" });
  s.addText("表达能力 · 总结与提炼", { x: M + 1.75, y: 3.62, w: 9.5, h: 0.42, margin: 0, fontFace: FONT, fontSize: 12.5, color: "7FB3CC", valign: "middle" });
  s.addText(String(pageNo), { x: W - M - 0.9, y: H - 0.62, w: 0.9, h: 0.32, margin: 0, fontFace: FONT, fontSize: 10, color: "7FB3CC", align: "right" });
  s.addNotes("第五部分，约 1 分钟。对应评分表 E 栏。");
}
...
pres.writeFile({ fileName: "开题报告_按评分表重排.pptx" }).then((f) => {
